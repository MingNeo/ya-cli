...
function buildProject(data, name, src, dest, done, others) {
  ...
  const upIgnores = ignDest => exists(path.join(dest, './' + ignDest)) && ignores.push(ignDest);
  console.log(others.save, path.join(dest, './index.js'));
  if (others.save) {
    // exists(path.join(dest, './src')) && ignores.push('src');
    ['src', 'index.js', 'README.md'].forEach(upIgnores);
  }
